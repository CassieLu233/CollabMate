...
function GitLab() {
  ...
  const getInformation=async () => {
    try {
      const res = await fetch(`${API_BASE}/gitlab/user`, {
      headers: {
  ...
  // unlink the connected GitLab Account
  const handleUnlink=async () => {
    try {
      const res = await fetch(`${API_BASE}/gitlab/unlink`, {
      method: "DELETE",
      headers: {
        "Authorization": `Bearer ${token}`,
      },
      ...
      {loading ? ( 
        ...
        <Box sx={{
          ...
          <Button variant="contained" sx={{
            ...
            width: 200}}
            onClick={() => {
                window.location.href = `${API_BASE}/gitlab/login?token=${token}`;
            }}
            endIcon={
